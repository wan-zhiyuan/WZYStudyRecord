...
const createElm = (e, t, a, o) => {
  ...
  appData.BUILD.updatable && null !== o && null !== s ? ((e, t, a, o) => {
    let s, n, l = 0, r = 0, p = 0, i = 0, d = t.length - 1, c = t[0], $ = t[d], D = o.length - 1, m = o[0], u = o[D];
    for (;l <= d && r <= D; ) if (null == c) c = t[++l]; else if (null == $) $ = t[--d]; else if (null == m) m = o[++r]; else if (null == u) u = o[--D]; else if (isSameVnode(c, m)) patch(c, m), 
    c = t[++l], m = o[++r]; else if (isSameVnode($, u)) patch($, u), $ = t[--d], u = o[--D]; else if (isSameVnode(c, u)) !appData.BUILD.slotRelocation || "slot" !== c.$tag$ && "slot" !== u.$tag$ || putBackInOriginalLocation(c.$elm$.parentNode, !1), 
    patch(c, u), e.insertBefore(c.$elm$, $.$elm$.nextSibling), c = t[++l], u = o[--D]; else if (isSameVnode($, m)) !appData.BUILD.slotRelocation || "slot" !== c.$tag$ && "slot" !== u.$tag$ || putBackInOriginalLocation($.$elm$.parentNode, !1), 
    ...
    l > d ? addVnodes(e, null == o[D + 1] ? null : o[D + 1].$elm$, a, o, r, D) : appData.BUILD.updatable && r > D && removeVnodes(t, l, d);
  ...
  const o = new (appData.BUILD.hydrateServerSide ? win.CustomEvent : CustomEvent)(t, a);
...
exports.attachShadow = e => {
  ...
  appData.BUILD.hydrateClientSide && appData.BUILD.shadowDom) for (;D < d.length; D++) registerStyle(d[D].getAttribute("sty-id"), d[D].innerHTML.replace(/\/\*!@([^\/]+)\*\/[^\{]+\{/g, "$1{"));
  e.map(e => e[1].map(a => {
    const l = {
      $flags$: a[0],
      $tagName$: a[1],
...
}, exports.setMode = e => modeResolutionChain.push(e), exports.setSupportsShadowDom = e => {
